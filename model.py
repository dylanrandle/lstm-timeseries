...
class Model():
    def __init__(self, args):
        ...
        self.inputs = tf.placeholder(tf.float32, shape=[args.batch_size, args.seq_len, 1])
        self.targets = tf.placeholder(tf.float32, shape=[args.batch_size, 1])
        ...
        rnn_outputs, rnn_states = tf.nn.dynamic_rnn(self.lstm_cell, self.inputs, initial_state=self.initial_state, time_major=False)
        self.last_state = rnn_states

        W = tf.Variable(tf.zeros([args.hidden_dim, 1]))
        b = tf.Variable(tf.zeros([1]))
        rnn_outputs = tf.reshape(rnn_outputs[-1][-1], [1, args.hidden_dim])
